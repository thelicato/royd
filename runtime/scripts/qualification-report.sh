  ...
  for version in $("$repo_root/android/scripts/version-list.sh"); do
    for arch in x86_64 arm64; do
      ...
      if [ -f "$file" ]; then
        ...
        security=not-run; graphics=not-run; logs=not-run; adb=not-run; binder=not-run; result=not-run
      fi
      printf '| %s | %s | %s | %s | %s | %s | %s | %s | %s | %s | %s | %s | %s |\n' \
        "$version" "$arch" "$host" "$image" "$boot" "$health" "$runtime" "$security" "$graphics" "$logs" "$adb" "$binder" "$result"
    done
  done
}
