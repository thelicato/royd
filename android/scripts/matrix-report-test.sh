...
grep -Fq '| 8.0 | `android-8.0.0_r36` | legacy | legacy | x86_64 | missing | not-run | not-run | not-run | legacy-configured |' "$report"
grep -Fq '| 15 | `android-15.0.0_r36` | modern | modern | arm64 | missing | not-run | not-run | not-run | baseline |' "$report"
grep -Fq '| 17 | `android-17.0.0_r1` | modern | modern | x86_64 | missing | not-run | not-run | not-run | configured |' "$report"
rows=$(grep -c '^| ' "$report")
[ "$rows" -eq 24 ] || {
...
ROYD_MATRIX_SOURCE_ROOT="$tmp" ROYD_MATRIX_OUTPUT="$report" "$script_dir/matrix-report.sh" >/dev/null
grep -Fq '| 15 | `android-15.0.0_r36` | modern | modern | x86_64 | present | pass | not-run | not-run | baseline |' "$report"
grep -Fq '| 15 | `android-15.0.0_r36` | modern | modern | arm64 | present | pass | not-run | not-run | baseline |' "$report"
...
RESULT
ROYD_MATRIX_SOURCE_ROOT="$tmp" ROYD_MATRIX_OUTPUT="$report" "$script_dir/matrix-report.sh" >/dev/null
grep -Fq '| 15 | `android-15.0.0_r36` | modern | modern | x86_64 | present | pass | pass | pass | package-validated |' "$report"
