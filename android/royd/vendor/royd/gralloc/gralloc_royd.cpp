...
int royd_gralloc_open(const hw_module_t* module, const char* name, hw_device_t** out_device) {
    ...
    if (strcmp(name, GRALLOC_HARDWARE_FB0) == 0) {
        ...
        device->common.module = const_cast<hw_module_t*>(module);
        device->common.close = royd_gralloc_close;
        const_cast<uint32_t&>(device->flags) = 0;
        const_cast<uint32_t&>(device->width) = static_cast<uint32_t>(width);
        const_cast<uint32_t&>(device->height) = static_cast<uint32_t>(height);
        const_cast<int&>(device->stride) = width;
        const_cast<int&>(device->format) = HAL_PIXEL_FORMAT_RGBA_8888;
        const_cast<float&>(device->xdpi) = static_cast<float>(dpi);
        const_cast<float&>(device->ydpi) = static_cast<float>(dpi);
        const_cast<float&>(device->fps) = static_cast<float>(fps);
        const_cast<int&>(device->minSwapInterval) = 0;
        const_cast<int&>(device->maxSwapInterval) = 1;
