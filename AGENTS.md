...
- Software graphics baseline: SwiftShader plus the graphics family selected by each pinned Android contract. Android 15 uses the repository-owned AIDL allocator V2, stable-C mapper V5, and composer3 V3 client-composition service. Other pinned versions retain the legacy `gralloc.royd` and `hwcomposer.default` path until researched separately. Experimental Android 10+ host GPU backends use AOSP Mesa plus minigbm and require explicit `/dev/dri` access.
