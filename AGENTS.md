...
- Active boot MVP scope: prioritise Android 15 x86_64 with the standard graphical software profile in privileged Docker until it reaches repeatable `sys.boot_completed=1`. Keep other versions, arm64, alternative profiles, host GPU and capability reduction configured, but defer their active validation until this baseline boots.
...
- Fstab-less Android boot: because OCI supplies royd's mounted Android partitions and the product has no device fstab or `mount_all` result, trigger Android's standard `nonencrypted` event during `late-fs` so the `main` and `late_start` service classes start normally.
...
- Android 15 Power HAL: install AOSP's `android.hardware.power-service.example` as the container baseline. The module provides its own init rules and source AIDL V6 VINTF fragment, reports unsupported headroom capabilities safely, and Android stable-AIDL release handling emits the latest frozen V5 wire version for `bp1a`. Do not duplicate that declaration in royd's central device manifest.
- Android 15 Audio HAL: install AOSP's `com.android.hardware.audio` APEX with a repository-owned primary speaker and microphone policy. Use the AOSP AIDL primary implementation in stub-input and stub-output mode so the container does not require an ALSA device. The APEX owns its init rules and source AIDL V3 VINTF fragments; Android stable-AIDL release handling emits frozen V2 declarations for `bp1a`. Do not duplicate those declarations in royd's central device manifest.
