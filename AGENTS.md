...
- Reference-host evidence: use `make runtime-reference-qualify` for support-oriented host validation. It collects host, image, runtime, memory, and optional capability-reduction evidence into a checksummed bundle. Verify retained evidence with `make runtime-reference-bundle-verify BUNDLE=...`; policy digest changes intentionally make older bundles stale.
